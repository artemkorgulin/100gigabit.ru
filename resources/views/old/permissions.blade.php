        ...
        <table class="users"> <!-- add materialize classes, as desired -->
            <thead>
                <tr class="collection-item">
                    <th>Идентификатор</th>
                    <th>Родитель</th>
                    <th class="filter-select filter-exact" data-placeholder="Укажите email">Показываемое имя</th>
                    <th>Описание</th>
                    <th>Операции</th>
                </tr>
            </thead>
            ...
            <tr class="tablesorter-ignoreRow collection-item">
                <th colspan="7" class="ts-pager form-horizontal">
                    ...
                    @can('create') <!-- РТПЧЕТСЕН РТБЧБ -->
                        ...
                        <div class="row right valign-wrapper add-user-button"><a class="col s12 m12 left-align" href="/permissions/create">Добавить роль</a></div>
                    @endcan
                </th>
            </tr>
            </tfoot>
            ...
            @foreach($permissions as $user)
                <tr class="collection-item">
                    <td><div class="col s12 m6 l1"><a href="/permissions/show/{{$user->id}}">({{$user->id}})</a></div></td>
                    <td><div class="col s12 m6 l3"><a href="/permissions/show/{{$user->id}}">{{$user->parent}}</a></div></td>
                    <td><div class="col s12 m6 l3"><a href="/permissions/show/{{$user->id}}">{{$user->display_name}}</a></div></td>
                    <td><div class="col s12 m6 l3"><a href="/permissions/show/{{$user->id}}">{{$user->description}}</a></div></td>
                    <td><div class="col s12 m6 l2"><a href="/permissions/delete/({{$user->id}})/"><i class="material-icons">delete</i></a></div></td>
                </tr>
            @endforeach
            </tbody>
        </table>
